...
const insertedTerm = async (termId, typeId, old) => {
    ...
    let term = await getTerm( typeId, termId ).catch(errorHadler);
    ...
    typeIds.map( async tid => {
        ...
        let slugs = await findTermParent( [type.slug, term.slug], termId, typeId );
        slugs = '/' + slugs.join('/');
...
setGlobalVar( 'insertedTerm', insertedTerm );
setGlobalVar( 'updatedTerm', insertedTerm );
