...
const getUsers = async query => {
    ...
    /**
     * Trigger to allow filtering and/or setting additional query parameter
     * in retrieving the list of users.
     *
     * @param {object} query
     */
    query = await Filter.apply( 'getUsersQuery', query );
    ...
    // Get total count
    let countQuery = _.extend( {}, query, {fields: 'ids'} ),
        count = await userQuery().getUsers( countQuery ).catch( err => {
            errorHandler(err);
            return 0;
        });
    ...
    let foundUsers = {foundUsers: count.length};

    return userQuery().getUsers(query)
        .then( async results => {
            foundUsers.users = results;

            if ( key ) {
                Cache.set( 'users', key, foundUsers );
            }
