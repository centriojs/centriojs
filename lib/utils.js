...
const runInVM = async (options, filename) => {
    let args = _.extend( {
        require: require,
        console: console,
        filename: filename,
    ...
    'use strict';
        ...
        output = require(filename)(options);
    
    `, sandbox );

    return sandbox;
};
setGlobalVar( 'runInVM', runInVM );
...
/**
 * Clear the cached data from `require.cache` object
 *
 * @param {string} filename
 */
const clearRequireCache = filename => {
    try {
        require(filename);
    } catch(e) {
        errorHandler(e);
...
setGlobalVar( 'clearRequireCache', clearRequireCache );
...
const removeHook = function(name, uniq) {
    ...
    if ( 'string' !== typeof uniq && uniq.name ) {
        uniq = uniq.name;
    }
