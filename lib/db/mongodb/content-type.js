...
class ContentTypeQuery {
    ...
    getTerms(query) {
        ...
        let {search} = query;
        // @todo: set search param here

        let addPropFilter = (name, value, compare) => {
            compare = compare || '=';
            let _filter = {};

            _filter.name = name.toString();
    ...
    __getProperty( propDb, objectId, name, isSingle ) {
        ...
        return this.__getProperties( propDb, filter )
            .then( results => {
                if ( ! results.length ) {
    ...
    __getProperties( propDb, query ) {
        ...
        if ( objectId ) {
            ...
            filter.objectId = {$in: objectId__in};
        }

        let {name, name__in} = query;
        if ( name ) {
            filter.name = name;
        } else if ( name__in ) {
            filter.name = {$in: name__in};
        }
