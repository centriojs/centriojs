...
class ContentTypeQuery {
    ...
    async updateContentTable( typeId, options ) {
        ...
        fields.map( field => {
            ...
            switch ( field ) {
                case 'title' :
                    columns.push('ADD `title` VARCHAR(155) NOT NULL');
                    break;
        ...
        // Drop comments table if comments is disabled
        // Create comments table if was not previously created.
    }
    ...
    updateContent(typeId, content) {
        ...
        return this.getContentQuery(typeId).update( filter, format );
    }
