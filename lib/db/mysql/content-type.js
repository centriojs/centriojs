...
class ContentTypeQuery {
    ...
    getContentTypes(query) {
        ...
        let {searchable, hasPage, hasArchive, hasComments} = query;
        if ( searchable ) {
        ...
        if ( hasArchive ) {
            where.push('`hasArchive` = ?');
            format.push(true);
        }

        if ( hasComments ) {
            where.push('`hasComments` = ?');
            format.push(true);
        }
    ...
    getTerms(query) {
        ...
        let {termsOf, property, properties} = query;
        if ( termsOf ) {
            property = {
                name: 'typeId',
                value: termsOf,
                compare: _.isArray(termsOf) ? 'IN' : false
            };
        }
    ...
    async searchContents(term, page, perPage) {
        term = '%' + term + '%';
        // get content of content types that are public and searchable
        let contentTypes = await this.getContentTypes({
            status: 'active',
            type: 'content',
            hasPage: true,
            searchable: true
        ...
        contentTypes.map( type => {
            let table = this.getContentQuery(type.ID);

            query.push('SELECT *, ? AS typeId FROM ?? WHERE `title` LIKE ?' );
            format.push(type.ID, table.table, term);
        });
        ...
        return this.dbManager.query( query, format );
    }
}
module.exports = db => {
