...
const createTables = async () => {
    ...
    columns = [
        ...
        '`status` ENUM ("public", "private", "admin") DEFAULT "admin"',
        '`type` VARCHAR(100) NOT NULL',
        'Index (`ID`, `userId`, `type`)'
    ];
    if ( isMongo ) {
        columns = {
            validator: {
        ...
        indexes = ['ID', 'userId', 'type'];
    }

    done = await dbManager.execQuery('user_activity').createTable( columns, indexes ).catch(errorHandler);
    if ( ! done || done.error ) {
        return reject( done.message || errMsg );
    }
    ...
    columns = [
        ...
        '`type` ENUM ("template", "module", "menu", "landing") DEFAULT "template"',
            ...
            validator: {
                $jsonSchema: {
                    ...
                    properties: {
                        ...
                        type: {
                            bsonType: 'enum',
                            enum: ['template', 'module', 'menu', 'landing'],
                            default: 'template'
                        },
                        location: {
                            bsonType: 'string',
...
class InstallRoutes {
    constructor(router) {
        ...
        router.setView( '/', this.setView.bind(this) );
    ...
    async setView() {
        ...
        // Check database connection, show database selection if none configured
        let done = dbManager.checkConnection().catch(errorHandler);
        if ( ! done || done.error ) {
            return this.dbSetup();
        }
