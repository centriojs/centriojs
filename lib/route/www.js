...
class Router {
    ...
    callback( cb ) {
        ...
        return async ( req, res ) => {
            ...
            if ( ! response.path ) {
                let routePath = _.routePath( this.isAdmin ? '/admin' + req.path : req.path );
                response.path = routePath.remove('page');
            }
            ...
            if ( response.description ) {
                metaData.push(`<meta name="description" value="${response.pageDescription}" />`);
            }
            ...
            _.keys(theme.stylesheets).map( key => {
                let css = theme.stylesheets[key],
                    src = css.src + '?v=' + css.version;

                let link = `<link rel="stylesheet" type="text/css" href="${src}" id="${key}-css" />`;
...
<body class="${bodyClass}">
    ...
    htmlHead(options) {
        ...
        if ( options.metaData ) {
            html = html.concat(options.metaData);
        }

        if ( options.stylesheets ) {
            html = html.concat(options.stylesheets);
        }
