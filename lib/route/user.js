...
class UserRouter {
    ...
    async getUsers( param ) {
        ...
        // Check search term in get request
        if ( search || $_GET.search ) {
            queryArgs.search = search || $_GET.search;
            response.searchTerm = search || $_GET.search;
        }

        if ( group || $_GET.group ) {
            queryArgs.group = group || $_GET.group;
        }

        let results = await getUsers(queryArgs);

        if ( results.foundUsers ) {
            response.foundUsers = results.foundUsers;
    ...
    async editProfile(param) {
        let id = param && param.id || currentUser.ID,
            response = {
                ...
                path: this.router.isAdmin ? '/admin/profile' : '/profile'
            };
