...
const updateContent = async content => {
    ...
    return contentTypeQuery().updateContent( typeId, contentColumns )
        .then( () => {
            ...
            appEvent.trigger( 'updatedContent', ID, typeId, old );
    ...
    return contentTypeQuery().deleteContent( typeId, ID )
        .then( async done => {
            ...
            if ( contentType.hierarchical ) {
                let children = await getContents({
                    typeId: typeId,
                    parent: ID
                });
...
const getCategoryBy = async (typeId, column, value) => {
    ...
    if ( cache ) {
        cache = await Filter.apply( 'getCategory', cache, typeId );
    ...
    return contentTypeQuery().getCategoryBy( typeId, column, value )
        .then( async results => {
            ...
            result = await Filter.apply( 'getCategory', result, typeId );
...
const updateCategory = async (typeId, category) => {
    ...
    return contentTypeQuery().updateCategory( typeId, category )
        .then( () => {
            ...
            appEvent.trigger( 'updatedCategory', ID, typeId, old );
...
const deleteCategory = async (typeId, ID) => {
    ...
    return contentTypeQuery().deleteCategory( typeId, ID )
        .then( () => {
            ...
            appEvent.trigger( 'deletedCategory', old, typeId );
...
const getTagBy = async (typeId, column, value) => {
    ...
    if ( cache ) {
        cache = await Filter.apply( 'getTag', cache, typeId );
    ...
    return contentTypeQuery().getTagBy( typeId, column, value )
        .then( async results => {
            ...
            result = await Filter.apply( 'getTag', result, typeId );
...
const getTags = async (typeId, query) => {
    ...
    if ( cache ) {
        ...
        for ( let i = 0; i < cache.length; i++ ) {
            ...
            tag = await Filter.apply( 'getTag', tag, typeId );
            tags.push(tag);
        }
...
const addTag = async (typeId, tag) => {
    ...
    if ( ! slug ) {
        tag.slug = _.toSlug(name);
    }

    return contentTypeQuery().addTag( typeId, tag)
        .then( id => {
            Cache.clearGroup( 'contentTags' + typeId );

            appEvent.on( 'insertedTag', id, typeId );
 ...
 * @returns {Promise<*>}
 */
const updateTag = async (typeId, tag) => {
    typeId = parseInt(typeId);
    ...
    return contentTypeQuery().updateTag( typeId, tag )
        .then( () => {
            Cache.clearGroup( 'contentTags' + typeId );
            Cache.clear( 'contentTagId' + typeId, ID );
            Cache.clear( 'contentTagSlug' + typeId, old.slug );

            appEvent.trigger( 'updatedTag', ID, typeId, old );
...
const deleteTag = async (typeId, ID) => {
    ...
    return contentTypeQuery().deleteTag( typeId, ID )
        .then( () => {
            ...
            appEvent.trigger( 'deletedTag', old, typeId );
