...
const addContentType = options => {
    ...
    if ( ! fields ) {
        if ( ! type ) {
            ...
            fields = ['name', 'description', 'slug', 'content_types'];
        }
    }
...
const deleteTypeProperty = function(typeId, name) {
...
const getTypeProperty = function( typeId, name ) {
    ...
    return contentTypeQuery().getProperty( typeId, name, isSingle )
        .then( results => {
            ...
            if ( name ) {
                Cache.set( 'typeProperty', name, value );
            } else {
...
const insertTerm = async term => {
    ...
    return contentTypeQuery().insertTerm( typeId, term )
        .then( async id => {
            ...
            await appEvent.trigger( 'insertedTerm', id );
...
const updateTerm = async term => {
    ...
    let old = await getTerm(ID).catch(errorHandler);
    if ( ! old || old.error ) {
        return reject( il8n('Invalid term type id.') );
    }
    ...
    return contentTypeQuery().getTerms(query)
        .then( async terms => {
            ...
            for ( let i = 0; i < terms.length; i++ ) {
                let term = await Filter.apply( 'getContent', term, typeId );
