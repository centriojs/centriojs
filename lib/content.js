 ...
 *
 * @param {int} typeId
 * @param {string} column               The name of the column field that the value corresponds at.
 *                                      Allowed columns are ID, slug.
 ...
 * @param {any} value                               Optional. If preset, will remove property that match the given value.
 * @returns {Promise<*>}
 */
 ...
 *
 * @param {object} query {
 *     @param {int} typeId                  Required. The tax type id.
 *     @param {int|array} termsOf           An id or array of id of content type of type content.
...
const getTerms = async query => {
    ...
    query = await Filter.apply( 'preGetTerms', query );
...
const setContentTerms = async (contentTypeId, termTypeId, contentId, termIds) => {
...
setGlobalVar( 'setContentTerm', setContentTerms );

const deleteContentTerms = async (contentTypeId, termTypeId, contentId, termIds) => {
    termIds = ! _.isArray( termIds ) ? [termIds] : termIds;
...
setGlobalVar( 'deleteContentTerm', deleteContentTerms );
